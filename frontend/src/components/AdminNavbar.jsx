...
import { Users, Store, User, LogOut, LogIn, UserPlus, MessageCircle } from "lucide-react";
...
const AdminNavbar = () => {
    ...
    const loadUnreadStatus = async () => {
        if (!loggedIn) return;
        const res = await api("/chat/admin/rooms", "GET");
    ...
    return (
        <header className="fixed top-0 left-0 right-0 bg-white/95 backdrop-blur border-b border-slate-200 z-40 shadow-sm">
            <div className="mx-auto max-w-7xl px-4 h-16 flex items-center gap-6">
                ...
                <Link to="/admin/dashboard" className="font-bold text-indigo-700 text-lg tracking-tight">
                    YourStore
                </Link>
                ...
                <div className="ml-auto flex items-center gap-4 relative">
                    {!loggedIn ? (
                        <div className="flex gap-4">
                            <Link to="/admin/login" className="h-9 w-9 rounded-full bg-slate-100 text-slate-700 flex items-center justify-center hover:bg-slate-200 transition-colors">
                                <LogIn size={16} />
                            </Link>

                            <Link to="/admin/signup" className="h-9 w-9 rounded-full bg-slate-100 text-slate-700 flex items-center justify-center hover:bg-slate-200 transition-colors">
                                <UserPlus size={16} />
                            </Link>
                        </div>
                    ) : (
